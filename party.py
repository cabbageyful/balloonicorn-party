...
def is_mel(name, email):
    """Is this user Mel?
        ...
        >>> is_mel('Melitpolski', 'MEL@ubermelon.com')
        True

        >>> is_mel('Mel Melitpolski', 'mel@test.com')
        True
    """
    name = name.lower()
    email = email.lower()
    if name == "mel melitpolski" or email == "mel@ubermelon.com":
        return True
    elif 'mel' in name or'mel' in email:
        return True
    else:
        return False
...
def most_and_least_common_type(treats):
    """Given list of treats, return {most, least} common types.
        >>> treat = [{'type':'dessert'},{'type':'dessert'},{'type':'appetizer'},{'type':'dessert'},{'type':'appetizer'},{'type':'drink'}]

        >>> most_and_least_common_type(treat)
        ('dessert', 'drink')

        >>> treat = [{'type':'dessert'},{'type':'appetizer'},{'type':'drink'}]

        >>> most_and_least_common_type(treat)
        ('dessert', 'dessert')

        >>> treat = [{'type':'dessert'}, {'type':'dessert'}, {'type':'dessert'}]

        >>> most_and_least_common_type(treat)
        ('dessert', 'dessert')

        >>> treat = [{'type':'dessert'},{'type':'dessert'},{'type':'appetizer'},{'type':'appetizer'}]
